...
3. 还有一方面，在视图渲染方面由于图像的渲染需要以60帧的刷新频率在屏幕上同时更新，在非主线程操作UI很难保证视图的同步更新。
...
6. GPU：GPU会等待VSyc信号的到来进行OpenGL渲染，将渲染好的数据输出到帧缓冲区
7. Dispaly：当VSyc信号到来时，显示器就会从帧缓冲区中取出画面，并输出到屏幕上。
