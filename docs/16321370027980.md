...
- Category 在刚刚编译完的时候，和原来的类是分开的，只有在程序运行起来后，通过 Runtime ，Category 和原来的类才会合并到一起。
    - 以方法列表作为例子，在合并过程中，首先会获取到分类结构体里的方法列表的个数count，然后根据count来对类的本身的方法列表进行内存移动，然后再将分类的方法列表补充道类的方法列表的最前面，因此，当分类跟类中有同名的方法，那么是会调用到分类的方法的，如果有多个分类有同名的方法，那么会调用到编译顺序最后的分类的方法。
...
二进制重排实际上就是将启动用到的函数方法尽可能地放在二进制文件加载的前面，并且是将函数符号地址连续的编译在一起，以减少缺页中断频率，加快启动速度。
...
```
可以看出，在有__block修饰符修饰的情况下：是一种指针传递，在Block内部可以通过指针访问到外部的变量，但是上述代码中的输出lgter的地址还是不一样的，因为在Block内部，是通过cself->lgter->forwarding->lgter，这获取的是结构体里的成员变量lgter，地址输出肯定是不一样的。
...
从图中可以看出OSSpinLock性能是最好的，但是由于对优先级反转问题的处理不当导致现在OSSpinLock已经被弃用了，而性能较好的就是GCD的信号量以及pthread_mutex，性能最差的就是@synchronized，@synchronized就是以性能相对较弱来换取简洁的语法。
