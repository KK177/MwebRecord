...
内部的实现是通过锁来实现的，在赋值或者取值前通过锁进行加锁，完成操作后就解锁。
...
- spinlock_t slock 锁，修改对象的引用计数以及weak表的时候会加锁防止多线程环境下出错。
...
- 当给A类添加KVO的时候，编译器会自动为A类创建一个派生类，并让A类的isa指针指向派生类，重写class方法，方法实现仍然返回原类
...
实现归档和解档，对象要遵守NSCoding协议，并且重写 -initWithCoder:（解档） 和 encodeWithCoder:（归档）方法
...
### GCD执行原理
- GCD有一个底层线程池，这个池中存放的是一个个的线程。之所以称为“池”，很容易理解出这个“池”中的线程是可以复用的，当一段时间后这个线程没有被调用的话，这个线程就会被销毁。注意：开多少条线程是由底层线程池决定的（线程建议控制再3~5条），池是系统自动来维护，不需要我们程序员来维护，而我们程序员需要关心的是什么呢？我们只关心的是向队列中添加任务，队列调度即可。
...
#### UIImage 缓存是怎么回事？
通过 imageNamed: 创建 UIImage 时，系统实际上只是在 Bundle 内查找到文件名，然后把这个文件名放到 UIImage 里返回，并没有进行实际的文件读取和解码。当 UIImage 第一次显示到屏幕上时，其内部的解码方法才会被调用（因为只有在执行imageView.image = image在赋值时，CATransaction会捕捉到imageView的图层变化，然后才会有后续的解码工作），同时解码结果会保存到一个全局缓存。
...
## 数据安全及加密
### 对称加密和非对称加密的区别？
1. 对称加密又称共享密钥加密，加密和解密都会用到同一个密钥，如果密钥被攻击者获得，此时加密就失去了意义。常见的对称加密算法有DES、3DES、AES、Blowfish、IDEA、RC5、RC6。
2. 非对称加密又称公开密钥加密，使用一对非对称的密钥，一把叫做私有密钥，另一把叫做公有密钥；公钥加密只能用私钥来解密，私钥加密只能用公钥来解密。常见的公钥加密算法有：RSA、ElGamal、背包算法、Rabin（RSA的特例）、迪菲－赫尔曼密钥交换协议中的公钥加密算法、椭圆曲线加密算法）。
...
3.开发者把证书请求文件（CSR）传到苹果服务器，苹果服务器用私钥A对CSR进行签名，得到一份数据包含公钥L及其签名，这里称这份数据为证书
4.在苹果后台申请AppID，配置好设备ID列表和App可使用的权限，再加上第三步得到的证书组成的数据用私钥A签名，把数据和签名一起组成Provisioning Profile文件（描述文件），下载到开发者的本机
...
[详细的签名机制](https://www.jianshu.com/p/7806ea264aea)
...
- Category 在刚刚编译完的时候，和原来的类是分开的，只有在程序运行起来后，通过 Runtime ，Category 和原来的类才会合并到一起。
    - 以方法列表作为例子，在合并过程中，首先会获取到分类结构体里的方法列表的个数count，然后根据count来对类的本身的方法列表进行内存移动，然后再将分类的方法列表补充到类的方法列表的最前面，因此，当分类跟类中有同名的方法，那么是会调用到分类的方法的，如果有多个分类有同名的方法，那么会调用到编译顺序最后的分类的方法。
...
#### 分类中有多个同名方法，那么怎么实现这些方法按一定顺序调用，而不是只调用编译顺序最后的方法？
可以在原类的load方法里面进行方法交换来实现分类的同名方法都调用。
[学习链接🔗](https://www.cnblogs.com/jgCho/p/11219936.html)
...
- 关于NTimer创建时API中的repeats参数，当传入YES，那么定时器就会在指定的时间里重复响应，但是如果传入的是NO，那么该定时器响应一次后就会自动调用invalidate方法。
- invalidate方法调用时RunLoop会移除对NSTimer的强引用。
...
- 当一个硬件事件(触摸/锁屏/摇晃等)发生后，首先由 IOKit.framework 生成一个 IOHIDEvent 事件并由 SpringBoard 接收。SpringBoard 只接收按键(锁屏/静音等)，触摸，加速，接近传感器等几种 Event，随后用 mach port 转发给需要的App进程。随后苹果注册的那个 Source1 就会触发回调，并调用 `_UIApplicationHandleEventQueue() `进行应用内部的分发。
    - SpringBoard是iOS 和 iPadOS负责管理主屏幕的基础程序，有时候主屏幕也被作为SpringBoard的代称。
...
- 这里的自动释放池就是为了延长runloop的生命周期，避免runloop被创建出来由于没有持有runloop源而立马被释放。
...
- 单例模式：单例保证了应用程序的生命周期内仅有一个该类的实例对象,而且易于外界访问。在ios sdk中,UIApplication, NSBundle, NSNotificationCenter, NSFileManager, NSUserDefault, NSURLCache等都是单例。
    - 简单讲一下利用gcd上的`dispatch_once`方法来实现单例。在这个方法里面主要传入一个`dispatch_once_t`的变量的地址，然后根据这个变量的值来判断是否需要执行block里的初始化对象的任务，当变量值为0时，执行任务，当变量为-1时跳过任务，当变量为其他值是阻塞线程。
    - 那么在多线程的环境下，当A线程来到`dispatch_once`方法，首先判断`dispatch_once_t`的变量值为0，那么执行任务，同时修改变量的值为其他，在此过程中如果有其他线程想要执行`dispatch_once`方法，那么当发现变量值为其他时，那么阻塞线程，当A线程执行完之后，将变量的值修改为-1，那么之后别的线程再来到时就要跳过此任务；所以说gcd实现的单例是线程安全的。
    - 单例模式和静态变量的区别
        - 单例模式下，在App的运行过程中，只会在内存中存在一个实例对象，而静态变量就不一定。
...
链接器就是靠着`重定位表`来知道哪些地方需要被重定位的。每个可能存在重定位的段都会有对应的重定位表。在链接阶段，链接器会根据重定位表中，需要重定位的内容，去全局符号表中找到地址并进行重定位。

在重定位表中，每个记录存储着该符号在原来段中的地址，当链接器链接完全部目标文件后，会根据这个符号去全局符号表中扫描出对应的记录，然后进行一些回调，将符号的地址写回到对应的段中。
...
### 编译顺序
在Xcode里面利用Command + 8快捷键可以看到编译流程的顺序；
文件之间的编译顺序主要是由依赖关系决定的，**但是依赖关系是怎么决定的暂时还没搞懂**
附上一张网图
![](media/16321370027980/16411315039415.jpg)

而对于两个新增加的文件，通过调试发现，后添加进来的文件会比先添加的先编译。

Xcode里面还使用到了增量编译，再次编译时只会编译那些修改过的文件或者新增加进行的文件。

[学习链接🔗](https://blog.csdn.net/Hello_Hwc/article/details/85226147)
...
### 生命周期
App的生命流程分为三大部分：
1. App的启动流程（包括pre-main以及mian两部分）
2. App的运行时生命周期

### 启动流程
- 解析info.plist文件
    - 加载相关信息，比如权限检查，以及沙盒的建立
- 加载Mach-O文件
...
![](media/16321370027980/16415402205274.jpg)

设置完之后，那么我们先将Appkill掉，比如删除掉，重新安装之后，再启动App，这时候就可以在控制台看见App启动即main函数之前所需要的时间。
...
#### 镜像文件的加载
在`_objc_init`中完成runtime的初始化之后，dyld就会对`load_images`方法进行监听，当加载镜像文件时，就会回调通知到dyld，执行`load_images`方法
...
##### load_images
在`load_images`方法中
- 首先加载当前类的分类，将分类结构体上的信息合并到类的结构体上
- 准备加载load函数，递归地加载当前类和还没有调用load方法的父类进入加载列表。在当前类加入加载列表前，会先将父类加入加载列表中，保证父类的load方法调用在子类的load方法之前，然后再加载分类的分类的load方法
- 维护一个结构体数组，专门用来保存要加载的类以及对应的load方法，结构体的内容为类以及load方法的imp，这也是为什么了，load方法是不走消息传递的，是直接根据函数地址调用的。
...
```
![-w770](media/16321370027980/16393621966551.jpg)
- 开始加载load方法：遍历维护的结构体数组，从中取出对应类的load方法的imp，直接进行调用。

##### load方法的调用顺序
- 当父类和子类都实现load函数时,父类的load方法执行顺序要优先于子类
- 当一个类未实现load方法时,不会调用父类load方法
- 类中的load方法执行顺序要优先于类别(Category)
- 当有多个类别(Category)都实现了load方法,这几个load方法都会执行,编译顺序后的会先执行，编译顺序前的会后执行
...
### 重新理一下关于App的启动
![](media/16321370027980/16415472090150.jpg)
从上图可以看出，App的启动分为pre-main以及main两个阶段。

从上图可以看出，广义上App的启动完成即从点击Appicon开始到ViewDidAppear结束即首页数据加载完毕，而从Apple的定义来说，即从代码层面，比较狭义的层面是指从进程被创建到第一个`CA::Transaction::commit()`（即第一个事务的提交）

那么怎么理解第一个事务的提交呢？
在main函数调用后，主线程的RunLoop已经初始化好了，第一个事务即首屏渲染中一组UI的修改被捕捉到这个事务里了，而在RunLoop即将进入休眠或者结束的时候，进行图片的解码，以及CPU的计算最后交给Render Server渲染。
...
1. 加载可执行文件，进而加载dyld
2. rebase&binding
    1. rebase：主要是调解镜像内部的指针，苹果使用了ASLR（地址空间布局随机化）计数。程序每次启动后地址都会随机变化，这样程序里所有代码地址都需要重新进行计算修复。
        1. 举个例子，就是当加载Mach-O文件到虚拟地址上时，会有一个随机偏移量A，本来文件里的指令、数据地址是从0开始偏移的，有了偏移量之后，那么就要将文件里的地址加上偏移量A。
    2. binding：修复指向外部的指针。比如在代码里使用到了NSLog函数，而NSLog函数是系统函数，在程序开始运行的时候App是不知道NSLog函数指针是什么，此时就需要dyld的bind技术从动态库中找到NSLog指针地址进行调用
3. Objc setup
    1. 进行OC的初始化，runtime就是在此处初始化的，将分类的信息整合到类的结构体中。
4. load&constructor&initialize
    1. 调用项目中的load方法，初始化C&C++的静态化变量，然后调用 constructor 函数

#### 在mian函数之后：
1. main函数
    1. 创建整个app的autoreleasepool，初始化window，app界面开始展示
2. LifeCyle
    1. 调用app的launch函数，在里面指定rootViewController，以及一些业务逻辑上的注册
3. First Frame
    1. 即App启动的第一个页面的首屏渲染，渲染完成，那么就代码App的启动完成了。

在大概了解了App的启动流程后，那我们在开发过程中如果想实现对App的启动优化，那么就可以思考上面的流程，看每个流程具体能优化什么。

而是否能真正的实现启动优化，那么根据就是App启动所需要的时间是否真的减少了，在App冷启动的情况下，启动时间相比原来减少了，那就真正的实现了启动优化。

那么接下来分析一下获取到App启动所需要的时间。
首先对于main函数之前的阶段，可以修改Xcode的scheme的run配置来达到App启动后在控制台输出启动时间的信息。

在 Xcode 中 Edit scheme -> Run -> Auguments 将环境变量 DYLD_PRINT_STATISTICS 设为1 。之后控制台会输出类似内容，我们可以清晰的看到每个耗时：
![](media/16321370027980/16415522680100.jpg)

而对于main函数阶段的时间统计 —— 暂时还没找到一个比较好的方案
- 可以再main函数以及根ViewController的viewDidAppear写个输出语句，然后在控制台观察两者的时间差，即为main函数阶段的时间

#### 二进制重排
我们在开发中可以利用二进制重排来对函数符号位置的重新排序，减少Mach-O文件载入内存时Page-Fault缺页中断频率。

二进制重排实际上就是将启动用到的函数方法尽可能地放在二进制文件加载的前面，并且是将函数符号地址==连续==的编译在一起，以减少缺页中断频率，加快启动速度。

在iOS上为16K为一页。
Page-Fault就是缺页中断：当App调用一个方法，发现该方法没有在内存上，那么就会触发缺页中断，阻塞整个App进程。操作系统会从磁盘上读这一页数据到内存上，然后再将其映射到虚拟内存上，（如果当时内存已满了，那么就需要页面置换算法置换出牺牲的页面）。
...
![](media/16321370027980/16415614966755.jpg)

二进制重排后
![](media/16321370027980/16415615058213.jpg)
method1、method5和method6全都集中在了page1，这样只需装载page1就可以了。相比之前少了page2和page3的装载。少了两次处理PageFault时间。这次消耗的时间是 1 * 0.5ms = 0.5ms。节省了1ms。

最后要明确一点是：对于函数符号的二进制重排，并不只是单纯地改变可执行文件的符号顺序，每个函数符号对应的地址其实都是改变的

#### 利用二进制重排后，怎么判断App启动时链接器的链接顺序是根据排序后的函数符号呢？
1. 根据编译的中产物linked_map.txt文件，该文件的 #Symbols记录了链接器在链接目标文件成可执行文件时的符号顺序，根据该文件内的符号顺序与收集到的符号顺序比对，比对正确那就说明二进制重排成功了。
2. 或者是利用Xocde里的Instruments工具里的System trace查看主线程的缺页中断次数 看是否减少了
3. 其次是分别打印出二进制重排前后的启动时间，看是否真的减少了。

#### 在开发中，工程的代码随着开发的进行会不断改变位置或者删减，之前已经排好的顺序有些可能失效了，那怎么解决？
比如在开发过程中，可能新增了一些首屏渲染时需要调用到的方法，那么之前已经排好的顺序其实就相当于失效了，设想一下：新增一个启动所需要调用到的方法，但是还是以旧的二进制顺序，那么就有可能新增一次缺页中断，如果新增了好几个方法，那就有可能新增好几次缺页中断。
因此，在实际开发过程中，可以定期进行一次二进制重排，确保page fault次数维持在一个较低的稳定的水平。

[学习链接](https://zhuanlan.zhihu.com/p/396550853)
...
但是要注意的是，如果有视图是UIControl的子类或者实例对象，当事件传递到它，尽管它不接受此事件，它也不会将事件传递下去。
...
但是要注意的是⚠️：
- 手势识别系列的方法是比touch事件系列的方法触发的更快，而且当手势识别出了之后调用touch事件cancel方法阻断响应链的传递了。
![](media/16321370027980/16384404640657.jpg)

因此，会出现当一个UIButton同时添加了点击手势以及添加了action，那么由于手势识别得更加地快，那么是会调用手势响应的方法。
但是我们也可以实现同时响应两个方法，就是设置手势的属性来改变它的表现`cancelsTouchesInView`，当设置为true之后，手势识别后也不会发送cancel到视图，那么就会同时出发两个响应的方法。
...
```
对上面转换后的代码进行分析
- `block`代码块被编译成了`__main_block_impl_0`结构体，将`block`的内容作为结构体的构造函数的参数传进去。
...
```
可以看出，在有__block修饰符修饰的情况下：是一种指针传递，在Block内部可以通过指针访问到外部的变量，但是上述代码中的输出lgter的地址还是不一样的，因为在Block内部，是通过cself->lgter->forwarding->lgter，这获取的是copy到堆上的结构体里的成员变量lgter，地址输出肯定是不一样的。

只是在__block修饰下，可以在block内部修改到外部的变量。
...
函数一般存在Mach-O 中 Data部分的_Text（segament）段中，当该页从磁盘上加载到内存，iOS系统还要对Text段进行解密，这样是为了防止IPA包被下载下来就可以看到源代码，这也就是为什么逆向中有个概念叫做“砸壳”，因此在page in过程中，不仅I/O操作耗费时间，而且解密也很浪费时间，而在iOS13后对这个过程进行了优化，Page in的时候不需要解密了。
...
##### 怎么hook的
这里是利用了LLVM内置的一个简单的代码覆盖率工具（SanitizerCoverage），它可以在函数、基本块和边缘插入对用户自定义函数的调用。
[文档](https://clang.llvm.org/docs/SanitizerCoverage.html)

##### 那么怎么验证是否真的改变了Mach-O文件的符号顺序呢？
这时候就要用到`Link Map`了。
`Link Map`记录了二进制文件的布局，其中存储了所有符号顺序，在 # Symbols: 部分，通过这个文件，我们可以看到编译顺序是否按照修改后的来。（链接器链接二进制文件是根据编译顺序来的）
...
### 关于弱网优化
#### 网络请求优化
- 制定合适的超时时间，比如在网络状态为2G、3G、4G、5G、wifi下设置不同的超时时间
- 多子模块请求的“延迟性”
当一个页面同时有多个模块时，优先展示请求数据量少的，对于请求数据量大的延后展示或者采取分页处理
- 对于固定模块添加缓存机制、或增量更新机制，在一定的有效时间内再次请求可以直接从缓存中取数据；也可以利用增量更新机制，在进行Http请求时添加一个if modified  since 字段，来获取从上次请求后更新的内容
- 优化HTTP请求
    - 尽量减少DNS查询，使用IP地址来避免DNS域名劫持等情况
    - 使用高版本的HTTP，从而减少数据包的大小，在HTTP/2中会对报文的首部和数据部分进行了压缩，从而大大减少包的大小
    - 使用CDN，内容分发网，来加速HTTP的请求，减少延迟。

#### 用户体验优化
- View可以优先展示请求好后的文字，当图片请求完成后再重新加载
- 当没网时，要有无网络提示提醒用户 —— AFNetworking的网络监听
- 加载图片或者页面时 添加提醒的视图
- 弱网加载失败，可以添加重新加载的按钮展示给用户
- 页面数据请求时，可以预先加载虚拟布局视图，比如知乎，点开文章加载数据时，会有一个虚拟布局视图

[学习链接🔗](https://www.jianshu.com/p/bd26837f2bd3)

## 锁🔒
在iOS中，锁的基本种类只有两种：自旋锁和互斥锁，其他的比如条件所，递归锁、信号量其实都是在这两个锁的基础上进一步封装的。

自旋锁：当线程获取不到锁的时候会反复检查锁是否可获取了，因此是一种忙等待，一直占用着CPU资源，当获取到锁时，线程会一直保持该锁，直到使用完毕主动释放。
可以看出：自旋锁避免了进程的上下文切换带来的开销，适用于线程阻塞时间很短的情况。

互斥锁：在线程获取锁但是没有获取到时，线程会进入休眠状态，等锁被释放时线程会被唤醒。

### 锁的性能对比
![](media/16321370027980/16416377155754.jpg)
从图中可以看出OSSpinLock性能是最好的，但是由于对优先级反转问题的处理不当导致现在OSSpinLock已经被弃用了，而性能较好的就是GCD的信号量以及pthread_mutex，性能最差的就是@synchronized，@synchronized就是以性能相对较差来换取简洁的语法。

### 关于OSSpinLock不安全的问题
首先要明白的是：OSSpinLock是一种自旋锁，在线程获取不到锁时是会处于忙等的状态，占用CPU的资源。

对于自旋锁的特性，那么当出现优先级反转问题时是处理不当的。

先解释一下优先级反转：
    就是一个高优先级任务间接被一个低优先级任务所抢先，使得两个任务的相对优先级被倒置。这往往出现在一个高优先级访问一个被低优先级任务正在使用的临界资源，从而阻塞了高优先级任务；同时，该低优先级任务被一个次高优先级的任务所抢先，从而无法及时地释放该临界资源。这种情况下，该次高优先级任务获得执行权。
    
那么在自旋锁的情况下，当出现了优先级反转问题，因为高优先级的一直占用着CPU，但这是处于一种忙等的状态，它一直获取不到被低优先级持有的资源，而低优先级由于优先级较低，无法获取到CPU的使用权限，导致无法释放锁。
因此，OSSpinLock是不安全的。

### 关于@synchronized
@synchronized的语法是非常简洁的，但是换来的就是性能上的损失。

```Objective-c
//OC上使用@synchronized加锁的代码
    @synchronized (self) {
        NSLog(@"1");
    }
```
而且@synchronized底层是对递归互斥锁的进一步封装，那么就说明一个线程是可以对对象进行多次加锁的，而线程之前就是互斥的。
...
#### 进一步解析
对于@synchronized加锁，其实是调用了objc_sync_enter(obj),而解锁其实是调用了objc_sync_exit(obj)。

而在objc_sync_enter(obj)和objc_sync_exit(obj)两个方法中，可以发现都通过id2Data(obj)方法获取到对应的SyncData，然后利用SyncData里的递归互斥锁进行加解锁。
![](media/16321370027980/16416422306638.jpg)

首先先对SyncData有个大概印象的了解。
![](media/16321370027980/16416423270528.jpg)


在通过id2Data方法获取obj对应的SyncData，主要根据以下步骤来进行。
1. 首先从当前线程的TLS快速缓存中获取SyncData
2. 第一步获取不到，那么就会从当前线程缓存中获取SyncCache结构体，然后再遍历该结构体获取到对应的SyncData
3. 都获取不到，那么就在全局哈希表SyncList中查找对应的SyncData
4. 都没有的话，那么就新建一个SyncData，同时保存到线程的快速缓存、缓存以及全局哈希表中。

所以说，根据传入的obj获取该对象对应的SyncData，在获取过程中首先根据二级缓存，二级缓存找不到那么就到全局哈希表中去查找。

关于设计的一些思考：
    首先@synchronized底层是一个递归互斥锁，那么说明在同一线程下可以进行多次加锁，那么设想，要是每次加锁都需要到全局映射表去获取，那是不是很麻烦？而且全局映射表在同一时间内可以被多条线程访问，那么说明，访问全局映射表还需要进行额外的加锁，那么带来的就是时间的消耗，因此，在线程上设计了二级缓存来大大提高加锁解锁的效率。
...
#### SDWebImage加载图片过程
1. 首先显示占位图
2. 在webImageCache缓存中寻找图片对应的缓存，此过程是以URL为数据索引在内存中查找的
3. 如果没有缓存，那么就通过md5处理过的key在磁盘中查找对应的数据，如果找到，那么就将数据从磁盘加载到内存中，并显示出来
4. 如果内存和磁盘都没有找到，就会向服务器发送请求，开始下载
5. 下载完成的图片会加入缓存中，并写进磁盘
6. 整个获取图片的过程是在子线程进行的，最后在主线程中显示。

#### SDWebImage缓存处理策略
SDWebImage使用的主要是二级缓存，内存缓存以及磁盘缓存。在请求图片前，首先会先去查询内存缓存，不命中再去查询磁盘缓存最后才会去发起请求。

关于Disk缓存清理策略：
 SDWebImage会在每次App结束或者退到后台的时候执行清理任务。清理缓存主要分为两步，第一步先清除掉过期的缓存文件，如果清除掉过期的缓存文件，空间还不够，那么就会按照文件时间从早到晚排序，先清除最早的缓存文件，直到剩余空间达到要求。
 在SDWebImage里Disk的缓存时间一般为一周，缓存超过一周的则默认为过期了，在App退到后台或者结束的时候就会将这些文件清除掉。
 而在默认情况，SDWebImage是没有设置最大缓存量的，但是我们可以在开发过程中手动设置`maxCacheSize`的值。



### AFNetworking
AFNetworking是封装的NSURLSession的网络请求，而YTKNetwork是在AFNetworking上的进一步封装。
两个网络库的主要区别有：
- AFNetworking提供的是集约式API，项目中的每一个请求都走这个入口，对外暴露了请求的URL、参数以及请求方式
- YTKNetwork提供的离散式API，项目中的每一个请求都要继承于YTKRequest，一个请求就相当于一个对象。这种方式，请求的URL、参数、请求方式等不暴露给外部，可见，这种方式拓展性会更好，但是会增加代码量，使包的体积变大。
- 相比于AFNetworking，YTKNetwork还提供了以下功能。
    - 支持按时间缓存网络请求内容
    - 支持按版本号缓存网络请求内容
    - 支持统一设置服务器和CDN的地址
    - 支持检查返回的JSON的合法性
    - 支持网络的批量请求和链式请求
![](media/16321370027980/16394695121593.jpg)

### MLeaksFinder
这是一个用于检测内存泄漏的第三方框架，区别于Instruments里的Leaks，MLeaksFinder提供的是一种实时监测的功能，当出现内存泄漏时，就会自动弹出弹窗告诉开发者，哪个页面发生了内存泄漏。

MLeaksFinder 目前能自动检测 UIViewController 和 UIView 对象的内存泄露，而且也可以扩展以检测其它类型的对象。

实现原理：
为基类NSObject添加一个方法`-willDealloc`方法，该方法的作用是，先用一个弱指针指向self，并在一小段时间（3秒）后，通过这个弱指针调用`-assertNotDealloc`，而 `-assertNotDealloc` 主要作用是直接中断言。

当认为某个对象应该要被释放了，在释放前调用`-willDealloc`方法，如果3秒后释放成功，那么weakSelf就指向nil，不会调用到`-assertNotDealloc` 方法，也就不会中断言，如果没有被释放（即泄漏了），-assertNotDealloc 就会被调用中断言。这样，当一个 UIViewController 被 pop 或 dismiss 时（我们认为它应该要被释放了），我们遍历该 UIViewController 上的所有 view，依次调 -willDealloc，若3秒后没被释放，就会中断言。
[学习链接🔗](http://wereadteam.github.io/2016/02/22/MLeaksFinder/)
...
1）先使用sd_layout为父类添加一系列的SDAutoLayoutModel类型的数组autoLayoutModelsArray

2） 在sd_layoutSubviewsHandle根据autoLayoutModelsArray，遍历每个子view，计算每个子view的frame

3） 在sd_resizeWithModel函数里面，生成子view的frame，如果子frame存在sd_bottomViewsArray和sd_rightViewsArray数组，就递归执行上面的步骤。


## CocoaPods
CocoaPods是一个负责管理iOS项目中第三方开源库的工具。

我们在项目中，当要使用第三方库时，如果不使用CocoaPods对第三方库进行统一管理，那么就需要我们下载第三方库，并将将他们导入到项目中，然后添加主项目跟第三方库之间的依赖，同时还要管理库的更新，当需要更新第三方库，那么就要重新下载并重新添加依赖，可见是非常耗时的。

那么在引入了CocoaPods之后，我们只需要的Profile文件里pod '需要的第三方库'，然后执行pod install，那么会将需要引入的第三方库都下载到Pods目录下，同时，CocoaPods会将所有的第三方库以target的方式组成一个名为Pods的工程，该工程就放在刚才新生成的Pods目录下。整个第三方库工程会生成一个名称为libPods.a 的静态库供我们的工程使用。

由于原来的工程设置已经被更改了，这时候我们直接打开原来的工程文件去编译是会报错的。我们的工程个第三方库所在的Pods工程会被以一个新生成的worksoace的形式组织和管理，方便我们直观的管理工程和第三方库。
