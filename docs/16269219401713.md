...
如果要在显示屏上显示内容
-------
，我们至少需要一块与屏幕像素数据量一样大的FrameBuffer，作为像素数据存储区域，GPU不停地将渲染完成后的内容放入FrameBuffer帧缓冲器中，而显示屏幕不断地从FrameBuffer中获取内容，显示实时内容。
...
而如果将** @property **声明的属性加上** @synthesize **修饰，那么我们可以同时重写属性的setter方法和getter方法
